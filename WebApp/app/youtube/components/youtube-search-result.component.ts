...
export class YoutubeSearchResultComponent {
  ...
  play(item: SearchItem) {
    this.player.play(<AudioItem>{ name: item.snippet.title, key: item.id.videoId });
  ...
  add(item: SearchItem) {
    this.session.queue.push(<AudioItem>{ name: item.snippet.title, key: item.id.videoId });
  }
